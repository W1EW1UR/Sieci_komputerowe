...
void add_new_conn(int fd)
{
    for (int i=0; i<MAX_CONNECTION; i++)	// Iteruje po wszystkich polaczeniach
        ...
        if (conn[i].status == CONNSTATE_IDLE)  // Sprawdza czy stan polaczenia jest "NIEOBECNY"
        {
            ...
            conn[i].name_pos = 0;    // Ustawiamy pozycje na 0 ??
...
void send_char(int i) 
{
  ...
  {
      conn[i].status = CONNSTATE_CLOSING;  // Jesli wszystko zostalo wyslane to zamykamy polaczenie 				[[[	NIEDOZWOLONE W PROJEKCIE	]]]
  }
}
